...
func (gc *GCloudContainer) SetNodePoolSize(name string, size int64) (err error) {
	...
	apiName := fmt.Sprintf("projects/%v/locations/%v/clusters/%v/nodePools/%v", gc.Client.Project, gc.Client.Region, gc.Client.Cluster, name)
...
func (gc *GCloudContainer) waitForOperation(operation *container.Operation) (err error) {
	...
	for {
		apiName := fmt.Sprintf("projects/%v/locations/%v/operations/%v", gc.Client.Project, gc.Client.Region, operation.Name)
